...
export default function Destaques({ produtos = [] }: { produtos: Produto[] }) {
  ...
  const formatarParcela = (valor: number) => {
    ...
    return `${parcelas}x de ${formatadorMoeda.format(valorParcela)}`;
  };
  ...
  const maxIndex = Math.max(0, produtos.length - itemsPerPage);
  ...
  }, [isMobile, maxIndex, produtos.length]);
  ...
  // se não houver produtos, não mostra o carrossel quebrado
  ...
  return (
    <section className="w-full max-w-[1300px] mx-auto px-4 py-16 overflow-hidden">
      ...
      <div className="flex items-center justify-center gap-4 mb-12">
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="#E11D48" stroke="#E11D48" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="w-6 h-6 md:w-8 md:h-8 lg:w-10 lg:h-10 xl:w-12 xl:h-12 2xl:w-14 2xl:h-14"><polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2"></polygon></svg>
        <h2 className="text-[30px] md:text-[34px] lg:text-[40px] xl:text-[42px] 2xl:text-[50px] font-extrabold uppercase tracking-wide text-white">Destaques</h2>
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="#E11D48" stroke="#E11D48" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="w-6 h-6 md:w-8 md:h-8 lg:w-10 lg:h-10 xl:w-12 xl:h-12 2xl:w-14 2xl:h-14"><polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2"></polygon></svg>
      </div>

      <div className="relative w-full md:px-12 lg:px-16">
        {/* seta esquerda */}
        <button onClick={prevSlide} className="absolute left-0 z-20 top-1/2 -translate-y-1/2 p-2 bg-white text-black rounded-full hover:bg-gray-200 transition hidden md:flex items-center justify-center cursor-pointer shadow-lg">
        ...
        <div className="overflow-hidden w-full py-4 -my-4">
          <div className="flex transition-transform duration-500 ease-out" style={{ transform: `translateX(-${currentIndex * getTranslateX()}%)` }}>
            {produtos.map((produto) => (
              <div key={produto.id} className="w-full md:w-1/2 lg:w-1/3 flex-none px-2 box-border">
                    ...
                    <div className="px-2 pt-2 pb-2 md:px-3 md:pt-3 md:pb-1 flex flex-col gap-0 md:gap-1 antialiased">
                      ...
                      <div className="mt-1 md:mt-3">
                        ...
                        <p className="text-[10px] md:text-[13px] text-gray-400 font-medium">
                          {formatarParcela(produto.price)} sem juros
                        </p>
                      </div>
        ...
        {/* seta direita */}
        <button onClick={nextSlide} className="absolute right-0 z-20 top-1/2 -translate-y-1/2 p-2 bg-white text-black rounded-full hover:bg-gray-200 transition hidden md:flex items-center justify-center cursor-pointer shadow-lg">
      ...
      <div className="flex lg:hidden justify-center items-center gap-2 mt-6">
        {Array.from({ length: maxIndex + 1 }).map((_, index) => (
          <button key={index} onClick={() => setCurrentIndex(index)} className={`w-3 h-3 rounded-full transition-colors duration-300 ${currentIndex === index ? "bg-rock-red" : "bg-zinc-600 hover:bg-zinc-500"}`} aria-label={`Ir para o slide ${index + 1}`} />
        ))}
      </div>

      <div className="flex justify-center mt-12">
        <Link href="/produtos" className="bg-rock-red hover:bg-red-700 text-white font-bold py-4 px-8 rounded-full transition-all duration-300 hover:scale-105 text-[18px] md:text-[23px]">
          Explorar produtos
        </Link>
      </div>
    </section>
