...
// importação do contexto global do carrinho para acessar os itens e funções de manipulação
import { useCart } from "@/src/providers/cart-context";

export default function PaginaCarrinho() {
  // estado para controlar a exibição do modal de sucesso ao finalizar a compra
  const [showModal, setShowModal] = useState(false);
  const router = useRouter();

  // desestruturação das funções e dados do carrinho gerenciados globalmente
  const {
    cartItems,
    increaseQuantity,
    decreaseQuantity,
  ...
  // exibe o modal de sucesso simulando o fim do fluxo de compra
  const handleFinalizarCompra = () => {
    setShowModal(true);
  };

  // redireciona o usuário para a home e limpa o carrinho após a compra com um leve delay
  ...
  // cálculo dinâmico dos valores do pedido baseado nos itens presentes no carrinho
  const subtotal = cartItems.reduce(
    (acc, item) => acc + item.price * item.quantity,
    0,
  );
  // taxa de frete fixa simbólica adicionada apenas se houver itens
  ...
  return (
    <main className="min-h-[75vh] bg-rock-dark pt-12 pb-20 px-4 relative">
      <div className="max-w-7xl mx-auto w-full">
        ...
        {/* renderização condicional: o que mostrar se o carrinho estiver vazio x com itens */}
        {cartItems.length === 0 ? (
          // layout de carrinho vazio com chamada para ação (voltar às compras)
          <div className="flex flex-col items-center justify-center bg-[#1A1A1A] rounded-2xl p-12 text-center border border-white/5 shadow-xl">
            ...
            <p className="text-gray-400 mb-8 max-w-md">
              Parece que você ainda não adicionou nenhum item da Caverna do Rock
              ao seu carrinho.
            </p>
            <Link
              href="/produtos"
              className="bg-rock-red hover:bg-red-700 text-white font-bold py-3 px-8 rounded-full transition-all hover:scale-105 active:scale-95 shadow-lg"
            >
              Continuar Comprando
            </Link>
          </div>
        ) : (
          // layout de carrinho com produtos usando grid responsivo
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            {/* área esquerda: lista detalhada dos produtos no carrinho */}
            <div className="lg:col-span-2 flex flex-col gap-4">
              {cartItems.map((item) => (
                <div
                  key={item.id}
                  className="flex flex-col sm:flex-row items-center justify-between bg-black/40 backdrop-blur-sm p-5 sm:p-6 rounded-2xl border border-white/10 shadow-xl gap-6"
                >
                  {/* foto, título, tamanho e preço individual do item */}
                  ...
                  <div className="flex items-center justify-between w-full sm:w-auto gap-6 sm:gap-8 border-t border-white/5 sm:border-t-0 pt-4 sm:pt-0 mt-2 sm:mt-0">
                    {/* botões de incrementar e decrementar quantidade */}
                    ...
                    {/* botão (lixeira) para remover o item completamente do carrinho */}
                    <button
            ...
            {/* área direita: caixa de resumo do pedido com posicionamento sticky (fixa ao rolar) */}
            <div className="bg-black/40 backdrop-blur-md p-8 rounded-2xl border border-white/10 shadow-2xl h-fit sticky top-28">
              ...
              <Link
                href="/produtos"
                className="block text-center mt-6 text-[16px] text-gray-400 hover:text-white underline transition-colors"
              >
                Continuar comprando
              </Link>
            </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm px-4">
          <div className="bg-[#1A1A1A] border border-white/10 p-8 rounded-3xl shadow-2xl max-w-sm w-full flex flex-col items-center text-center">
            ...
            <p className="text-gray-400 text-[16px] mb-8 leading-relaxed">
              Seu pedido foi confirmado. Prepare-se para vestir o som que você
              ama! 🤘
            </p>
